...
P_rk = 2; %Presion de Referencia en Kg/cm^2
P_r = 101325+P_rk*98066.5; %Presion de Referencia en Pa
n = 8; %numero de camaras neumaticas
...
%Parametros del Semgemnto inicial sin camara neumatica
b(1)=0;
L(1)=0;
d(1)=0;
e(1)=0;
...
V_o = zeros(1,n+1)%Volumen inicial de la camara 

for i=1:n+1
V_o(i) = L(i)*d(i)*e(i);
end

t=((P_r*7*V_o(3)/P_o)-(V_o(3)*7))*1900/Qf;%Calculo de instancia donde la presion interna alcanza la referencia
...
for i=1:n+1    
W(i) = (P_o*(((V_o(i)+(Qf)*t)^2)-(V_o(i)^2)))/(2*V_o(i));
...
ylabel('Y Axis (m)');
grid on;
title 'Soft Actuator Trajectory';
axis([0 0.11 -0.09 0.02]);
pause(0.1);
